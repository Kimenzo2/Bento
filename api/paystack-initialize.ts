...
export default async function handler(
  ...
  try {
    const response = await fetch(PAYSTACK_INIT_URL, {
      ...
      body: JSON.stringify({
        email,
        plan,        // Paystack uses the plan's amount automatically
        callback_url,
        metadata: {
          ...metadata,
          custom_fields: [
    ...
    if (!response.ok || !data.status) {
      console.error('[paystack-initialize] Paystack API error:', data);
    ...
    console.error('[paystack-initialize] Server error:', error);
