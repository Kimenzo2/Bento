...
async function verifyGreenRoom() {
      ...
      content.split('\n').forEach((line) => {
        ...
        if (match) {
          ...
          if (key.startsWith('VITE_GREEN_ROOM_API_KEY')) {
            console.log(
              `   Found ${key}: ${value.substring(0, 5)}...${value.substring(value.length - 4)} (Length: ${value.length})`
            );
          }
          envVars[key] = value;
        }
  ...
  try {
    ...
    const resp = await fetch('https://generativelanguage.googleapis.com', { method: 'HEAD' }).catch(
      (e) => e
    );
