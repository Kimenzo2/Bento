...
const GamificationHub: React.FC<GamificationHubProps> = ({ gameState, setMode }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 pb-24 animate-fadeIn">
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div className="flex-1 w-full">
            ...
            <div className="w-full h-6 bg-cream-base rounded-full border border-peach-soft overflow-hidden">
              ...
              >
                <div className="absolute inset-0 bg-surface/20 animate-[pulse_2s_infinite]"></div>
              </div>
            </div>
            ...
            {gameState.dailyChallenges.map((challenge) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3
                      className={`font-bold text-sm ${challenge.completed ? 'text-charcoal-soft line-through opacity-50' : 'text-charcoal-soft'}`}
                    >
                      {challenge.title}
                    </h3>
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map((rank) => (
              <div key={rank} className="flex items-center gap-3 p-2">
                ...
                <div className="w-8 h-8 bg-peach-light/50 rounded-full"></div>
      ...
      {/* Badges Grid */}
      <div className="mt-8 bg-surface rounded-3xl border border-peach-soft p-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
          ...
          {[1, 2, 3].map((i) => (
            <div key={`locked-${i}`} className="flex flex-col items-center text-center opacity-30">
              <div className="w-20 h-20 rounded-2xl bg-peach-soft/30 flex items-center justify-center mb-3">
                <Lock className="w-6 h-6" />
              </div>
              <div className="h-4 w-16 bg-peach-light/50 rounded mt-1"></div>
            </div>
          ))}
        </div>
      </div>
