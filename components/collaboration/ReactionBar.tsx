        ...
        if (result.success && result.data) {
          ...
          setReactions((prev) => {
            ...
            if (existing) {
              const updated = prev
                .map((r) =>
                  r.type === type ? { ...r, count: Math.max(0, r.count + (added ? 1 : -1)) } : r
                )
                .filter((r) => r.count > 0);
      ...
      <div className={`flex items-center ${config.gap} flex-wrap`}>
        ...
        {!isExpanded &&
          sortedReactions.slice(0, maxVisible).map(({ type, count }) => (
            ...
            >
              <span
                className={`${config.emoji} ${animatingReaction === type ? 'animate-bounce' : ''}`}
              >
...
export const FloatingReaction: React.FC<FloatingReactionProps> = ({ type, x, y, onComplete }) => {
  useEffect(() => {
