...
const ChallengeCard: React.FC<ChallengeCardProps> = ({
  ...
  const _handleVote = async (submission: ChallengeSubmission) => {
    ...
    try {
      ...
      // Update local state
      setSubmissions((prev) =>
        prev.map((s) => (s.id === submission.id ? { ...s, vote_count: s.vote_count + 1 } : s))
      );
      onVote?.(submission);
    } catch (error) {
          ...
          <div className="relative flex flex-col md:flex-row items-start md:items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span
                  className={`${getStatusColor()} text-white text-xs px-2 py-0.5 rounded-full font-bold`}
                >
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        {/* Icon */}
        <div
          className={`w-12 h-12 rounded-xl ${difficultyConfig.color} flex items-center justify-center`}
        >
