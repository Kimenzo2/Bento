    ...
    content: (
      <div className="p-6 bg-gray-900 text-white rounded-xl h-64 flex flex-col items-center justify-center border border-white/10 relative overflow-hidden">
        ...
        <div className="flex gap-4 mb-4">
          <div className="w-16 h-20 bg-gray-800 rounded-lg border border-gray-700 flex items-center justify-center text-xs text-cocoa-light/60">
            Watercolor
          </div>
          <div className="text-2xl font-light text-cocoa-light">+</div>
          <div className="w-16 h-20 bg-gray-800 rounded-lg border border-gray-700 flex items-center justify-center text-xs text-cocoa-light/60">
            Cyberpunk
          </div>
        </div>
        ...
        <p className="mt-4 text-xs text-cocoa-light/60">Seamlessly blend any two art styles</p>
      </div>
    ),
  },
  {
      ...
      <div className="p-6 bg-gray-900 text-white rounded-xl h-64 flex flex-col items-center justify-center border border-white/10">
        <div className="flex gap-2 mb-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="w-16 h-16 rounded-full bg-indigo-500/20 border border-indigo-400/30 flex items-center justify-center"
            >
        ...
        <p className="mt-4 text-xs text-cocoa-light/60">
          AI remembers your character's face across pages
        </p>
      </div>
    ),
  },
    ...
    content: (
      <div className="p-6 bg-surface border border-peach-soft rounded-xl h-64 relative">
        ...
        <div className="space-y-2">
          <div className="h-2 w-full bg-peach-soft/30 rounded" />
          <div className="h-2 w-3/4 bg-peach-soft/30 rounded" />
          <div className="h-2 w-5/6 bg-peach-soft/30 rounded" />
        </div>
...
export const FeatureExplorer: React.FC<FeatureExplorerProps> = ({ tier }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8 bg-surface rounded-2xl p-8 border border-peach-soft">
      <div className="w-full md:w-1/3 space-y-2">
        ...
        {features.map((feature) => {
          ...
          return (
            ...
            >
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${isActive ? `bg-${tier.accentColor}-200 text-${tier.accentColor}-700` : 'bg-peach-soft/30 text-cocoa-light'}`}
              >
