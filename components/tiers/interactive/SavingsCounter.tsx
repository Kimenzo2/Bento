...
export const SavingsCounter: React.FC<SavingsCounterProps> = ({ tier }) => {
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(animate);
  }, [mode, tier.price.monthly, comparisons]);
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 right-0 w-64 h-64 bg-linear-to-br ${tier.gradient} opacity-5 blur-3xl rounded-full -translate-y-1/2 translate-x-1/2`}
      />
      <div
        className={`absolute bottom-0 left-0 w-48 h-48 bg-linear-to-tr ${tier.gradient} opacity-5 blur-2xl rounded-full translate-y-1/2 -translate-x-1/3`}
      />
        ...
        <div className="w-full md:w-1/2 space-y-4 md:space-y-6">
          <div>
            ...
            <p className="text-charcoal-soft/70 text-xs md:text-sm">
              Stop overpaying specifically for content creation. Compare Genesis {tier.name} against
              traditional methods.
            </p>
          </div>
            ...
            {(Object.keys(comparisons) as ComparisonMode[]).map((key) => {
              ...
              return (
                <Button
                  ...
                  onClick={() => setMode(key)}
                  className={`
                    flex gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-1.5 md:py-2 rounded-full font-medium
                    ${
                      isActive
        ...
        <div className="w-full md:w-1/2 bg-white/60 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/50 text-center relative overflow-hidden group">
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-4 divide-x divide-charcoal-soft/10">
            <div className="space-y-1">
              <div className="text-charcoal-soft/60 text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                Monthly Savings
              </div>
              <div
                className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl font-heading font-black bg-linear-to-br ${tier.gradient} bg-clip-text text-transparent`}
              >
            ...
            <div className="h-8 flex-1 mx-4 flex items-end justify-end gap-1 opacity-50">
              {[20, 35, 45, 60, 55, 70, 80, 85, 90, 100].map((h, i) => (
                <motion.div
                  ...
                  className={`w-1 rounded-t-sm bg-linear-to-t ${tier.gradient}`}
                />
              ))}
            </div>
          </div>
