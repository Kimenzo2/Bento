...
export const SavingsCounter: React.FC<SavingsCounterProps> = ({ tier }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br ${tier.gradient} opacity-5 blur-3xl rounded-full -translate-y-1/2 translate-x-1/2`}
      />
      <div
        className={`absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-tr ${tier.gradient} opacity-5 blur-2xl rounded-full translate-y-1/2 -translate-x-1/3`}
      />
        ...
        <div className="w-full md:w-1/2 bg-white/60 backdrop-blur-md rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/50 text-center relative overflow-hidden group">
          ...
          <div className="grid grid-cols-2 gap-2 md:gap-4 divide-x divide-charcoal-soft/10">
            <div className="space-y-1">
              ...
              <div
                className={`text-xl sm:text-2xl md:text-3xl lg:text-4xl font-heading font-black bg-gradient-to-br ${tier.gradient} bg-clip-text text-transparent`}
              >
