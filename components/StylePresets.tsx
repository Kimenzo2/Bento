...
const BUILT_IN_PRESETS: StylePreset[] = [
  {
    ...
    style:
      'Soft watercolor illustration with gentle brushstrokes, pastel colors, whimsical and dreamy atmosphere',
  ...
  },
  {
    ...
    style:
      'Pixar-style 3D rendered illustration, expressive characters with big eyes, vibrant colors, cinematic lighting',
  ...
  },
  {
    ...
    style:
      "Classic children's book illustration, detailed hand-drawn style, warm earthy colors, cozy and nostalgic",
  ...
  },
  {
    ...
    style:
      'Anime-style illustration, expressive eyes, dynamic poses, clean line art with cel shading',
  ...
  },
  {
    ...
    style:
      'Paper cut-out illustration style, layered textured paper, handcrafted look, depth through shadows',
  ...
  },
  {
    ...
    style:
      'Oil painting style, rich textures and brushstrokes, dramatic lighting, Renaissance-inspired composition',
  ...
  },
  {
    ...
    style:
      'Epic fantasy illustration, dramatic lighting, magical glow effects, detailed environments, heroic poses',
...
export function useStylePresets() {
  ...
  const addPreset = useCallback(
    (preset: Omit<StylePreset, 'id' | 'createdAt' | 'usageCount' | 'isBuiltIn'>) => {
  ...
  const updatePreset = useCallback((id: string, updates: Partial<StylePreset>) => {
    setPresets((prev) => prev.map((p) => (p.id === id && !p.isBuiltIn ? { ...p, ...updates } : p)));
  }, []);
...
export const PresetCard: React.FC<PresetCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <h4
        className={`font-semibold mb-1 ${isSelected ? 'text-white' : 'text-gray-900 dark:text-white'}`}
      >
        {preset.name}
      </h4>
      ...
      {preset.description && (
        <p
          className={`text-sm mb-2 line-clamp-2 ${isSelected ? 'text-white/80' : 'text-gray-500 dark:text-gray-400'}`}
        >
          {preset.description}
        </p>
      )}
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          className="relative bg-white dark:bg-gray-900 rounded-3xl border border-peach-soft w-full max-w-4xl max-h-[90vh] md:max-h-[85vh] overflow-hidden flex flex-col"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {displayPresets.map((preset) => (
                  <PresetCard
                    ...
                    onDelete={preset.isBuiltIn ? undefined : deletePreset}
