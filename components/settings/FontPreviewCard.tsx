...
import type { FontPairing } from '../../src/types/fonts';
...
const FontPreviewCard: React.FC<FontPreviewCardProps> = ({ pairing, isActive, onSelect }) => {
  ...
  useEffect(() => {
    const loadPreviewFonts = async () => {
      try {
        await Promise.all([loadFont(pairing.headingFont), loadFont(pairing.bodyFont)]);
        setIsLoaded(true);
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        ...
        {/* Preview Area */}
        <div
          className={`space-y-2 mb-4 transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
        >
          <h3
            className="text-2xl font-bold text-charcoal-soft leading-tight"
            style={{
              fontFamily: `'${pairing.headingFont.family}', ${pairing.headingFont.fallback}`,
            }}
          >
