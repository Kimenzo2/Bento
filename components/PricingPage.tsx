...
import { Check, Star, Zap, Briefcase, Crown, X } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-end">
          {tiers.map((tier, index) => (
            ...
            >
              ...
              {tier.priceMonthly === 0 ? (
                <button className={`w-full py-3 rounded-xl font-heading font-bold transition-all mb-8 ${tier.buttonColor}`}>
                  Start Creating Free
                </button>
              ) : (
                <PaystackIntegration
                  className={`w-full py-3 rounded-xl font-heading font-bold transition-all mb-8 ${tier.buttonColor}`}
                  email="user@example.com"
                  amount={Math.round((isAnnual ? tier.priceAnnual : tier.priceMonthly) * 100)}
