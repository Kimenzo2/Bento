...
- No durable state — refreshing the page during book generation lost all
  progress
...
- All AI calls go through the **Mastra Hono server** (`mastra/server.ts`,
  port 4111)
...
Vercel serverless function (function timeout limits are incompatible with
durable workflow state).

### Mastra Cloud entry path (required)

Mastra Cloud expects the agent entry file at one of these paths:

- `src/mastra/index.ts`
- `src/mastra/index.js`

This repository keeps the main Mastra implementation at `mastra/index.ts`, so a
compatibility file is included at `src/mastra/index.ts` that re-exports from
`../../mastra/index`.

If you refactor folders later, keep `src/mastra/index.ts` present (or update
your project to match Mastra Cloud's expected path) to avoid
`MASTRA_ENTRY_FILE_NOT_FOUND` during deploy.
...
After deploying, add your Mastra server hostname to the `connect-src` directive
in `vercel.json`. The current config already allows `*.railway.app` and
`*.onrender.com`. For custom domains, add them manually:
...
The 10-step `bookGenerationWorkflow` replaces the previous single-call
generation:
...
- Approve → `POST /api/workflows/book-generation/resume` with
  `{ approved: true }`
...
- Trace context propagated via `userId`, `tier`, `workflowId` from request
  headers
...
- **Legacy VITE\_ keys** are still required for the fallback path
- **Mastra Studio** requires `DefaultExporter` + local storage (not available on
  Vercel)
- **WebSocket streaming** for workflow progress is SSE-based (works, but
  stateless servers need sticky routing)

---

_Last updated: Phase 6 — March 2026_
