...
# Agent setup for `Genesis`

This is a deepsec scanning workspace. Project `Genesis` was just registered
(target: `..`). Setup is incomplete — `data/Genesis/INFO.md`
...
2. **Fill in `data/Genesis/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id Genesis
pnpm deepsec process --project-id Genesis
```
