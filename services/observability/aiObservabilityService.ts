 ...
 */
export function getRecentTraces(count = 50): AITrace[] {
  return [...traces].sort((a, b) => b.startTime - a.startTime).slice(0, count);
}
...
export function getMetrics(): AIMetrics {
  ...
  const operationBreakdown: Record<
    string,
    { count: number; avgLatency: number; successRate: number }
  > = {};
  ...
  for (const [op, opTraces] of operationGroups) {
    ...
    operationBreakdown[op] = {
      ...
      avgLatency:
        opLatencies.length > 0 ? opLatencies.reduce((a, b) => a + b, 0) / opLatencies.length : 0,
      successRate: opTraces.length > 0 ? (opSuccessful.length / opTraces.length) * 100 : 0,
    };
  }
