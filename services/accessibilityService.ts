import { type GrokMessage, callAPI } from './grokService';
...
export async function batchGenerateAltText(
  ...
  for (let i = 0; i < pages.length; i += 3) {
    ...
    const promises = batch.map((page) =>
      generateImageAltText(page.text, artStyle, page.pageNumber, context).then((result) => ({
...
export const screenReaderAnnounce = {
  ...
  pageChange: (pageNumber: number, totalPages: number) => {
    screenReaderAnnounce.announce(`Page ${pageNumber} of ${totalPages}`, 'polite');
  },
  ...
  generationProgress: (step: string, progress: number) => {
    screenReaderAnnounce.announce(`${step}: ${Math.round(progress)}% complete`, 'polite');
  },
};
