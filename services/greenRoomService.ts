...
import { callGreenRoomAI } from './greenRoomAIService';
...
function parseJSON<T>(text: string, fallback: T): T {
  try {
    ...
    if (
      firstBracket !== -1 &&
      lastBracket !== -1 &&
      (firstSquare === -1 || firstBracket < firstSquare)
    ) {
...
export const greenRoomService = {
  ...
  initializePersona(character: Character, project: BookProject): CharacterPersona {
    ...
    const personality = [...(character.personalityTraits || []), ...(character.traits || [])];
    ...
    const goals =
      character.goals ||
      (character.coreIdentity
        ? ([character.coreIdentity.greatestDesire].filter(Boolean) as string[])
        : []);
    ...
    const fears =
      character.fears ||
      (character.coreIdentity
        ? ([character.coreIdentity.greatestFear].filter(Boolean) as string[])
        : []);

    // Build visual description from all appearance data
    const visualDescription = character.appearance || character.visualTraits || '';
    ...
    if (character.coreIdentity) {
      if (character.coreIdentity.flaw) {
        extractedFacts.push({
          id: generateUUID(),
          key: 'character_flaw',
          value: character.coreIdentity.flaw,
          source: 'profile',
          confidence: 1.0,
      ...
      if (character.coreIdentity.strength) {
        extractedFacts.push({
          ...
          value: character.coreIdentity.strength,
          source: 'profile',
          confidence: 1.0,
          extractedAt: Date.now(),
        });
      ...
      if (character.coreIdentity.lie) {
        extractedFacts.push({
          id: generateUUID(),
          key: 'lie_believed',
          value: character.coreIdentity.lie,
          source: 'profile',
          confidence: 1.0,
          extractedAt: Date.now(),
        });
      ...
      if (character.coreIdentity.truth) {
        extractedFacts.push({
          id: generateUUID(),
          key: 'truth_to_learn',
          value: character.coreIdentity.truth,
          source: 'profile',
          confidence: 1.0,
          extractedAt: Date.now(),
        });
    ...
    if (character.innerConflicts?.length) {
      extractedFacts.push({
        id: generateUUID(),
        key: 'inner_conflicts',
        value: character.innerConflicts.join('; '),
        source: 'profile',
        confidence: 1.0,
        extractedAt: Date.now(),
      });
    ...
    if (character.voiceProfile?.catchphrases?.length) {
      extractedFacts.push({
        id: generateUUID(),
        key: 'catchphrases',
        value: character.voiceProfile.catchphrases.join(' | '),
        source: 'profile',
        confidence: 1.0,
        extractedAt: Date.now(),
      });
  ...
  buildCharacterSystemPrompt(
    ...
    const facts = persona.extractedFacts.map((f) => `- ${f.key}: ${f.value}`).join('\n');
...
Extract any new facts about ${characterName} from this exchange.`,
  ...
  async saveSession(userId: string, session: GreenRoomSession): Promise<void> {
    try {
      const { error } = await supabase.from('green_room_sessions').upsert({
        id: session.id,
        user_id: userId,
