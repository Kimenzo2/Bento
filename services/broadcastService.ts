...
import {
  ...
  type BroadcastServiceCallbacks,
...
import { LRUCache, throttle } from './performanceOptimizations';
...
class BroadcastService {
  ...
  private throttledViewerUpdate = throttle(async (sessionId: string, count: number) => {
    await supabase.from('broadcast_sessions').update({ viewer_count: count }).eq('id', sessionId);
  ...
  async joinBroadcast(sessionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    const { error } = await supabase.from('broadcast_viewers').upsert({
   ...
   */
  subscribeToBroadcast(sessionId: string, callbacks: BroadcastServiceCallbacks): RealtimeChannel {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: { new: any; old?: any }) => {
          ...
          if (
            callbacks.onViewerCountChange &&
            payload.new.viewer_count !== payload.old?.viewer_count
          ) {
  ...
  async togglePinMessage(messageId: string, isPinned: boolean): Promise<void> {
    await supabase.from('broadcast_messages').update({ is_pinned: isPinned }).eq('id', messageId);
  }
  ...
  async markQuestionAnswered(messageId: string): Promise<void> {
    await supabase.from('broadcast_messages').update({ is_answered: true }).eq('id', messageId);
  }
  ...
  async addBookmark(
    ...
    const { error } = await supabase.from('broadcast_bookmarks').insert({
      session_id: sessionId,
      user_id: user.data.user.id,
