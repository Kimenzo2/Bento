...
import { LRUCache, deduplicateRequest, throttle } from './performanceOptimizations';
...
class CollaborationService {
   ...
   */
  async createSession(name = 'Creative Session'): Promise<ServiceResult<CollaborationSession>> {
  ...
  async getChallenge(challengeId: string): Promise<Challenge | null> {
    ...
    if (data?.top_submissions) {
      data.top_submissions.sort(
        (a: ChallengeSubmission, b: ChallengeSubmission) =>
  ...
  subscribeToSession(
    ...
    channel.on(
      'presence',
      { event: 'join' },
      ({ newPresences }: { newPresences: PresenceUser[] }) => {
    ...
    channel.on(
      'presence',
      { event: 'leave' },
      ({ leftPresences }: { leftPresences: PresenceUser[] }) => {
      ...
      if (status === 'SUBSCRIBED') {
        ...
        if (user.data.user) {
          ...
          channel.track({
            ...
            avatar_url:
              profile?.avatar_url ||
              `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.data.user.id}`,
