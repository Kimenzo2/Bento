...
class VersionControlService {
  ...
  async deleteVersion(versionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    const { error } = await supabase.from('visual_versions').delete().eq('id', versionId);
  ...
  private async pruneOldVersions(visualId: string): Promise<void> {
    ...
    if (data) {
      for (const version of data) {
        await supabase.from('visual_versions').delete().eq('id', version.id);
      }
    }
  }
  ...
  async deleteBranch(branchId: string): Promise<{ success: boolean; error?: string }> {
    const { error } = await supabase.from('visual_branches').delete().eq('id', branchId);
  ...
  private async buildForkTree(visualId: string): Promise<Array<{ visual: any; forks: any[] }>> {
  ...
  async getFamilyTree(visualId: string): Promise<FamilyTree> {
    ...
    for (const branch of branches) {
      const branchVersions = versions.filter((v) => v.branch_id === branch.id);
   ...
   */
  async getLineage(visualId: string): Promise<
    Array<{
