...
import type { BookProject, RemixCredits, RemixableWorld, WorldFork } from '../types';
...
export const remixService = {
  ...
  createWorldFromProject(
    ...
    const lore = [project.synopsis, ...(project.storyBible?.globalThemes || [])].join('\n\n');
  ...
  async publishWorld(world: RemixableWorld): Promise<{ success: boolean; error?: string }> {
    try {
      const { error } = await supabase.from('remixable_worlds').upsert({
        id: world.id,
        name: world.name,
