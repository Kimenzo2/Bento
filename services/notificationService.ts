...
class NotificationService {
  ...
  async createBulkNotifications(
    ...
    const { error, count } = await supabase.from('notifications').insert(insertData);
  ...
  async clearAll(): Promise<void> {
    ...
    await supabase.from('notifications').delete().eq('user_id', user.data.user.id);
  }
  ...
  private addToBatch(
    ...
    if (!this.batchTimeouts.has(type)) {
      const config = NOTIFICATION_BATCH_CONFIG[type];
      const timeout = setTimeout(() => this.flushBatch(type, userId), config?.batchWindow || 60000);
  ...
  async notifyChallenge(
    ...
    await this.createNotification(userId, type, titles[type], messages[type], {
