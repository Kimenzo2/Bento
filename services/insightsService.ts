...
class InsightsService {
  ...
  async generateInsights(userId: string): Promise<UserInsights | null> {
    const [creativeMetrics, styleAnalysis, timeAnalysis, engagementPatterns, recommendations] =
      await Promise.all([
    ...
    // Upsert insights
    const { data, error } = await supabase.from('user_insights').upsert(insights).select().single();
  ...
  private async calculateCreativeMetrics(userId: string): Promise<CreativeMetrics> {
    ...
    const totalReactions =
      sharedVisuals?.reduce(
        (sum: number, v: { reaction_count?: number }) => sum + (v.reaction_count || 0),
        0
      ) || 0;
    const totalRemixes =
      sharedVisuals?.reduce(
        (sum: number, v: { remix_count?: number }) => sum + (v.remix_count || 0),
        0
      ) || 0;
    ...
    const avgTime = timingData?.length
      ? timingData.reduce(
          (sum: number, d: { generation_time_ms: number }) => sum + d.generation_time_ms,
          0
        ) / timingData.length
      : 0;
  ...
  private async analyzeTimePatterns(userId: string): Promise<TimeOfDayAnalysis> {
    ...
    return {
      ...
      most_active_day: mostActiveDay ? Number.parseInt(mostActiveDay[0]) : null,
    };
  }
  ...
  private async analyzeEngagement(userId: string): Promise<EngagementPattern> {
    ...
    const totalEngagements = visuals.reduce(
      (sum: number, v: VisualData) => sum + (v.reaction_count || 0) + (v.comment_count || 0),
      0
    );
   ...
   * Get trending styles
   */
  async getTrendingStyles(limit = 10): Promise<TrendingStyle[]> {
    const { data, error } = await supabase
  ...
  async updateTrendingStyles(): Promise<void> {
    ...
    if (trendingData.length > 0) {
      await supabase.from('trending_styles').upsert(trendingData, { onConflict: 'style_name' });
    }
  }
   ...
   */
  async getComparisonToAverage(
    userId: string
  ): Promise<Record<string, { user: number; average: number; difference: number }>> {
    const userInsights = await this.getUserInsights(userId);
    ...
    return Object.fromEntries(
      Object.entries(metrics).map(([key, val]) => [
        key,
        {
          ...val,
          difference:
            val.average > 0 ? Math.round(((val.user - val.average) / val.average) * 100) : 0,
        },
      ])
    );
  }
