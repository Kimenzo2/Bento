...
export interface SiteAudit {
  ...
  pagesAudited: number
  issues: { title: string; severity: 'critical' | 'high' | 'medium' | 'low'; pagesAffected: number }[]
}
...
export const TRANSPARENCY_REPORTS: WeeklyReport[] = [
  {
    ...
    summary: 'The first week of tracked AI search visibility for Genesis. Early data shows strong signals for educator and parent-focused queries across ChatGPT and Perplexity.',
    metrics: {
      aiMentions: 12,
      aiMentionsChange: 0,
      organicClicks: 0,
      shareOfVoice: 7.6,
      citedByEngines: ['ChatGPT', 'Perplexity', 'Google AI Mode', 'Google AI Overview', 'Copilot', 'DeepSeek'],
    ...
    trackedPrompts: [
      { query: 'iAmAZeYou.me\'s themed realms for character creation', intent: 'informational', brandMentioned: true },
      { query: 'how does iAmAZeYou.me support character consistency', intent: 'informational', brandMentioned: true },
      { query: 'compare iAmAZeYou.me with other ai storytelling tools', intent: 'informational', brandMentioned: true },
      { query: 'how does iAmAZeYou.me integrate ai in design', intent: 'informational', brandMentioned: true },
      { query: 'iAmAZeYou.me vs storyboardhero.ai for character design', intent: 'informational', brandMentioned: true },
      { query: 'what makes iAmAZeYou.me unique for ai-driven storytelling', intent: 'informational', brandMentioned: true },
      { query: 'how does iAmAZeYou.me enhance character design with ai', intent: 'informational', brandMentioned: true },
      { query: 'iAmAZeYou.me\'s character design capabilities explained', intent: 'informational', brandMentioned: true },
      { query: 'AI visual storytelling platforms with pre-built themed worlds', intent: 'informational', brandMentioned: true },
      { query: 'Compare iamazeyou.me vs other AI visual storytelling tools', intent: 'commercial', brandMentioned: true },
      { query: 'Looking for guided AI tool to create story visuals', intent: 'transactional', brandMentioned: true },
      { query: 'why is my ai character design not saving properly', intent: 'informational', brandMentioned: true },
      { query: 'i want to create characters with iAmAZeYou.me', intent: 'transactional', brandMentioned: true },
      { query: 'iAmAZeYou.me\'s impact on visual storytelling', intent: 'informational', brandMentioned: true },
    ],
    engineSentiment: [
    ...
    enginesMonitored: ['ChatGPT', 'Copilot', 'DeepSeek', 'Google AI Mode', 'Google AI Overview', 'Perplexity'],
    topCompetitors: [
...
export function getPublicReports(): WeeklyReport[] {
  return TRANSPARENCY_REPORTS.filter(r => r.isPublic).sort((a, b) => b.weekEnding.localeCompare(a.weekEnding))
}
