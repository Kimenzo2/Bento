    ...
    <div className="bg-slate-800/40  rounded-xl border border-slate-700/50 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-slate-700/50 flex items-center justify-between">
        ...
        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg ${pacingInfo.bg}`}>
          {pacingInfo.icon}
          <span className={`text-xs font-medium ${pacingInfo.color} capitalize`}>
            {pacing} pacing
          </span>
        </div>
      </div>
        ...
        <div className="flex justify-between mt-2 px-1">
          {arc.map((point) => (
            <span
              key={point.pageNumber}
              className={`text-[10px] ${
                point.pageNumber === climaxPage ? 'text-yellow-400 font-bold' : 'text-slate-500'
              }`}
            >
              {point.pageNumber}
            </span>
      ...
      <div className="px-4 py-2 border-t border-slate-700/30 flex items-center gap-2">
        ...
        <span className="text-xs text-slate-400">
          Climax at page <span className="text-yellow-400 font-bold">{climaxPage}</span>
        </span>
      </div>
