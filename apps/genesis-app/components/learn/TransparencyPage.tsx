  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <svg width="76" height="76" viewBox="0 0 76 76">
        ...
        />
        <text x="38" y="42" textAnchor="middle" fontSize="18" fontWeight="600" fill={S.text}>
          {score}
        </text>
      </svg>
      <p className="text-xs font-medium" style={{ color: S.textMuted }}>
        {label}
      </p>
    </div>
  );
  ...
  return (
    <div className="border-t" style={{ borderColor: S.border }}>
      ...
      >
        ...
        {open ? (
          <ChevronUp className="w-4 h-4" style={{ color: S.textMuted }} />
        ) : (
          <ChevronDown className="w-4 h-4" style={{ color: S.textMuted }} />
        )}
      </button>
    ...
    >
      ...
      <div className="p-6 sm:p-8">
        ...
        <h2
          ...
          style={{ fontFamily: S.fontSerif, fontWeight: 400 }}
        >
          {report.headline}
        </h2>
        <p className="leading-relaxed" style={{ color: S.textMuted }}>
          {report.summary}
        </p>
      </div>
      ...
      {/* Metrics strip */}
      <div
        className="border-t border-b px-6 sm:px-8 py-4 flex flex-wrap gap-6"
        style={{ borderColor: S.border, backgroundColor: S.bg }}
      >
        {report.metrics.shareOfVoice != null && (
          <div>
            <p className="text-lg font-semibold" style={{ color: S.text }}>
              {report.metrics.shareOfVoice}%
            </p>
          ...
          <div className="flex items-center gap-2">
            ...
            {report.metrics.aiMentionsChange != null && report.metrics.aiMentionsChange !== 0 && (
              <span
                ...
                style={{ color: report.metrics.aiMentionsChange > 0 ? S.green : S.red }}
              >
                {report.metrics.aiMentionsChange > 0 ? (
                  <TrendingUp className="w-3 h-3" />
                ) : (
                  <TrendingDown className="w-3 h-3" />
                )}
                {report.metrics.aiMentionsChange > 0 ? '+' : ''}
                {report.metrics.aiMentionsChange}%
              </span>
            )}
          </div>
        )}
        ...
        {report.metrics.citedByEngines && report.metrics.citedByEngines.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <p className="text-xs ml-1" style={{ color: S.textMuted }}>
              Cited by
            </p>
          </div>
        )}
      ...
      {/* Highlights */}
      <div className="p-6 sm:p-8">
        <h3 className="text-sm font-semibold mb-3" style={{ color: S.text }}>
          Highlights
        </h3>
      ...
      {report.siteAudit && (
        ...
        >
          <div className="flex flex-wrap gap-8 mb-6">
            <ScoreRing
              score={report.siteAudit.aeoScore}
              label="AEO Score"
              color={
                report.siteAudit.aeoScore >= 80
                  ? S.green
                  : report.siteAudit.aeoScore >= 50
                    ? S.yellow
                    : S.red
              }
            />
            <ScoreRing
              score={report.siteAudit.contentScore}
              label="Content"
              color={
                report.siteAudit.contentScore >= 80
                  ? S.green
                  : report.siteAudit.contentScore >= 50
                    ? S.yellow
                    : S.red
              }
            />
            <ScoreRing
              score={report.siteAudit.technicalScore}
              label="Technical"
              color={
                report.siteAudit.technicalScore >= 80
                  ? S.green
                  : report.siteAudit.technicalScore >= 50
                    ? S.yellow
                    : S.red
              }
            />
                ...
                {report.siteAudit.issues.map((issue, i) => (
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ backgroundColor: S.yellowBg, color: S.yellow }}
                    >
                      {issue.severity}
                    </span>
                    <span className="text-xs" style={{ color: S.textMuted }}>
                      {issue.pagesAffected} page{issue.pagesAffected !== 1 ? 's' : ''}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-2">
            {report.trackedPrompts.map((prompt, i) => (
              ...
              >
                ...
                <span className="flex-1" style={{ color: S.text }}>
                  {prompt.query}
                </span>
                <span
      ...
      {report.sourceUrl && (
        <div
          className="border-t px-6 sm:px-8 py-4 flex items-center justify-between"
          style={{ borderColor: S.border, backgroundColor: S.bg }}
        >
          <p className="text-xs" style={{ color: S.textMuted }}>
            Data sourced from Searchable
          </p>
          <a
...
const TransparencyPage: React.FC = () => {
  ...
  usePageSEO({
    ...
    description:
      'Weekly reports on how Genesis performs in AI search engines including ChatGPT, Perplexity, and Google AI Overviews.',
      ...
      <section className="border-b" style={{ borderColor: S.border, backgroundColor: S.surface }}>
        <div className="max-w-4xl mx-auto px-5 sm:px-8 py-16 sm:py-24">
          ...
          <p className="text-lg leading-relaxed max-w-xl" style={{ color: S.textMuted }}>
            Weekly AI search visibility reports &mdash; published every Monday. We track how Genesis
            appears in ChatGPT, Perplexity, Google AI Overviews, and organic search.
          </p>
        </div>
      </section>
      ...
      <main className="max-w-4xl mx-auto px-5 sm:px-8 py-16 sm:py-20">
        {reports.length === 0 ? (
          ...
          >
            ...
            <p className="text-lg font-medium mb-2" style={{ color: S.text }}>
              Reports publish every Monday
            </p>
      ...
      >
        <div className="max-w-5xl mx-auto px-5 flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <div className="flex gap-4">
            <Link to="/learn" className="hover:underline" style={{ color: S.textMuted }}>
              Learn
            </Link>
