import type { Theme } from '../types/theme';

export const themes: Theme[] = [
  {
    id: 'genesis',
    name: 'Genesis Classic',
    description: 'Original warm & cozy theme',
    colors: {
      primary: ['#FF9B71', '#FFD93D'],
      accent: ['#FFD93D', '#FF9B71'],
      background: '#FFF8E7',
      text: '#5A5A5A',
      textLight: '#8B7E74',
      border: '#FFE4CC',
      shadow: '255, 155, 113, 0.15',
    },
    cssVariables: {
      '--primary': '#FF9B71',
      '--primary-hover': '#E8845A',
      '--accent': '#FFE4CC',
      '--color-primary-start': '#FF9B71',
      '--color-primary-end': '#FFD93D',
      '--color-accent-start': '#FFD93D',
      '--color-accent-end': '#FF9B71',
      '--color-background': '#FFF8E7',
      '--color-surface': '#FFFFFF',
      '--color-text': '#5A5A5A',
      '--color-text-light': '#8B7E74',
      '--color-border': '#FFE4CC',
      '--color-shadow': '255, 155, 113',
    },
    darkCssVariables: {
      '--primary': '#E87D56',
      '--primary-hover': '#D06A43',
      '--accent': '#3D302B',
      '--color-primary-start': '#E87D56',
      '--color-primary-end': '#E5C02B',
      '--color-accent-start': '#E5C02B',
      '--color-accent-end': '#E87D56',
      '--color-background': '#1A1412',
      '--color-surface': '#2A201D',
      '--color-text': '#EAE0D5',
      '--color-text-light': '#A89F91',
      '--color-border': '#3D302B',
      '--color-shadow': '255, 155, 113',
    },
  },
  {
    id: 'aurora',
    name: 'Aurora Scholar',
    description: 'Northern Lights Academic',
    colors: {
      primary: ['#4A148C', '#E91E63'],
      accent: ['#FFB74D', '#9C27B0'],
      background: '#F8F5FF',
      text: '#2D1B4E',
      textLight: '#6A4F8B',
      border: '#E1BEE7',
      shadow: '74, 20, 140, 0.1',
    },
    cssVariables: {
      '--primary': '#6A1FA8',
      '--primary-hover': '#561890',
      '--accent': '#E1BEE7',
      '--color-primary-start': '#4A148C',
      '--color-primary-end': '#E91E63',
      '--color-accent-start': '#FFB74D',
      '--color-accent-end': '#9C27B0',
      '--color-background': '#F8F5FF',
      '--color-surface': '#FFFFFF',
      '--color-text': '#2D1B4E',
      '--color-text-light': '#6A4F8B',
      '--color-border': '#E1BEE7',
      '--color-shadow': '74, 20, 140',
    },
    darkCssVariables: {
      '--primary': '#7C4DFF',
      '--primary-hover': '#6B3DE8',
      '--accent': '#362554',
      '--color-primary-start': '#7C4DFF',
      '--color-primary-end': '#F48FB1',
      '--color-accent-start': '#FFCC80',
      '--color-accent-end': '#CE93D8',
      '--color-background': '#130C1C',
      '--color-surface': '#1E152D',
      '--color-text': '#E2D8F0',
      '--color-text-light': '#A594BD',
      '--color-border': '#362554',
      '--color-shadow': '74, 20, 140',
    },
  },
  {
    id: 'ocean',
    name: 'Ocean Academy',
    description: 'Deep Learning Waters',
    colors: {
      primary: ['#006064', '#00ACC1'],
      accent: ['#4DD0E1', '#80DEEA'],
      background: '#E0F7FA',
      text: '#00363A',
      textLight: '#00838F',
      border: '#B2EBF2',
      shadow: '0, 96, 100, 0.1',
    },
    cssVariables: {
      '--primary': '#006064',
      '--primary-hover': '#004F52',
      '--accent': '#B2EBF2',
      '--color-primary-start': '#006064',
      '--color-primary-end': '#00ACC1',
      '--color-accent-start': '#4DD0E1',
      '--color-accent-end': '#80DEEA',
      '--color-background': '#E0F7FA',
      '--color-surface': '#FFFFFF',
      '--color-text': '#00363A',
      '--color-text-light': '#00838F',
      '--color-border': '#B2EBF2',
      '--color-shadow': '0, 96, 100',
    },
    darkCssVariables: {
      '--primary': '#26C6DA',
      '--primary-hover': '#1BAFC1',
      '--accent': '#1A424A',
      '--color-primary-start': '#26C6DA',
      '--color-primary-end': '#80DEEA',
      '--color-accent-start': '#B2EBF2',
      '--color-accent-end': '#E0F7FA',
      '--color-background': '#0A191C',
      '--color-surface': '#12272B',
      '--color-text': '#D5EFEF',
      '--color-text-light': '#85AFA8',
      '--color-border': '#1A424A',
      '--color-shadow': '0, 96, 100',
    },
  },
  {
    id: 'forest',
    name: 'Forest Wisdom',
    description: 'Ancient Knowledge Grove',
    colors: {
      primary: ['#2E7D32', '#66BB6A'],
      accent: ['#AED581', '#FFE57F'],
      background: '#F1F8E9',
      text: '#1B5E20',
      textLight: '#558B2F',
      border: '#C5E1A5',
      shadow: '46, 125, 50, 0.1',
    },
    cssVariables: {
      '--primary': '#2E7D32',
      '--primary-hover': '#236228',
      '--accent': '#C5E1A5',
      '--color-primary-start': '#2E7D32',
      '--color-primary-end': '#66BB6A',
      '--color-accent-start': '#AED581',
      '--color-accent-end': '#FFE57F',
      '--color-background': '#F1F8E9',
      '--color-surface': '#FFFFFF',
      '--color-text': '#1B5E20',
      '--color-text-light': '#558B2F',
      '--color-border': '#C5E1A5',
      '--color-shadow': '46, 125, 50',
    },
    darkCssVariables: {
      '--primary': '#66BB6A',
      '--primary-hover': '#52A356',
      '--accent': '#28462C',
      '--color-primary-start': '#66BB6A',
      '--color-primary-end': '#A5D6A7',
      '--color-accent-start': '#C5E1A5',
      '--color-accent-end': '#FFF59D',
      '--color-background': '#0F1A12',
      '--color-surface': '#18291B',
      '--color-text': '#DBEFE0',
      '--color-text-light': '#8EAB94',
      '--color-border': '#28462C',
      '--color-shadow': '46, 125, 50',
    },
  },
  {
    id: 'nebula',
    name: 'Nebula Mind',
    description: 'Cosmic Curiosity',
    colors: {
      primary: ['#1A237E', '#7B1FA2'],
      accent: ['#E91E63', '#FFD54F'],
      background: '#EDE7F6',
      text: '#12005E',
      textLight: '#5E35B1',
      border: '#D1C4E9',
      shadow: '26, 35, 126, 0.1',
    },
    cssVariables: {
      '--primary': '#4527A0',
      '--primary-hover': '#361F84',
      '--accent': '#D1C4E9',
      '--color-primary-start': '#1A237E',
      '--color-primary-end': '#7B1FA2',
      '--color-accent-start': '#E91E63',
      '--color-accent-end': '#FFD54F',
      '--color-background': '#EDE7F6',
      '--color-surface': '#FFFFFF',
      '--color-text': '#12005E',
      '--color-text-light': '#5E35B1',
      '--color-border': '#D1C4E9',
      '--color-shadow': '26, 35, 126',
    },
    darkCssVariables: {
      '--primary': '#5C6BC0',
      '--primary-hover': '#4A58A8',
      '--accent': '#2A2244',
      '--color-primary-start': '#5C6BC0',
      '--color-primary-end': '#AB47BC',
      '--color-accent-start': '#EC407A',
      '--color-accent-end': '#FFE082',
      '--color-background': '#100C1A',
      '--color-surface': '#1A152A',
      '--color-text': '#DED8EB',
      '--color-text-light': '#938BA3',
      '--color-border': '#2A2244',
      '--color-shadow': '26, 35, 126',
    },
  },
  {
    id: 'sunset',
    name: 'Sunset Scholar',
    description: 'Golden Hour Learning',
    colors: {
      primary: ['#BF360C', '#FF6F00'],
      accent: ['#FFB74D', '#FFD54F'],
      background: '#FFF3E0',
      text: '#3E2723',
      textLight: '#8D6E63',
      border: '#FFCCBC',
      shadow: '191, 54, 12, 0.1',
    },
    cssVariables: {
      '--primary': '#BF360C',
      '--primary-hover': '#A32D09',
      '--accent': '#FFCCBC',
      '--color-primary-start': '#BF360C',
      '--color-primary-end': '#FF6F00',
      '--color-accent-start': '#FFB74D',
      '--color-accent-end': '#FFD54F',
      '--color-background': '#FFF3E0',
      '--color-surface': '#FFFFFF',
      '--color-text': '#3E2723',
      '--color-text-light': '#8D6E63',
      '--color-border': '#FFCCBC',
      '--color-shadow': '191, 54, 12',
    },
    darkCssVariables: {
      '--primary': '#FF7043',
      '--primary-hover': '#E85C30',
      '--accent': '#4A2E1F',
      '--color-primary-start': '#FF7043',
      '--color-primary-end': '#FFCA28',
      '--color-accent-start': '#FFE082',
      '--color-accent-end': '#FFF59D',
      '--color-background': '#1C120C',
      '--color-surface': '#2D1C12',
      '--color-text': '#EFE5DE',
      '--color-text-light': '#AC9B8F',
      '--color-border': '#4A2E1F',
      '--color-shadow': '191, 54, 12',
    },
  },
  {
    id: 'midnightclassic',
    name: 'Midnight Classic',
    description: 'Clean contrast, zero distraction',
    colors: {
      primary: ['#1A1A1A', '#2D2D2D'],
      accent: ['#E8E8E8', '#E8E8E8'],
      background: '#FAF7F4',
      surface: '#FDFAF7',
      text: '#1F1C1A',
      textLight: '#6B635B',
      border: '#EDE4D9',
      shadow: '31, 28, 26, 0.12',
    },
    darkColors: {
      primary: ['#F0F0F0', '#DCDCDC'],
      accent: ['#252525', '#252525'],
      background: '#0D0D0D',
      surface: '#161616',
      text: '#F0F0F0',
      textLight: '#666666',
      border: '#252525',
      shadow: '240, 240, 240, 0.12',
    },
    cssVariables: {
      '--background': '#FAF7F4',
      '--surface': '#FDFAF7',
      '--card': '#FFFBF5',
      '--popover': '#FFF8F0',
      '--foreground': '#1F1C1A',
      '--muted': '#6B635B',
      '--border': '#EDE4D9',
      '--primary': '#1A1A1A',
      '--primary-hover': '#2D2D2D',
      '--accent': '#E8E8E8',
      '--color-background': '#FAF7F4',
      '--color-surface': '#FDFAF7',
      '--color-border': '#EDE4D9',
      '--color-text': '#1F1C1A',
      '--color-text-light': '#6B635B',
      '--color-primary-start': '#1A1A1A',
      '--color-primary-end': '#2D2D2D',
      '--color-accent-start': '#E8E8E8',
      '--color-accent-end': '#E8E8E8',
      '--color-shadow': '31, 28, 26',
    },
    darkCssVariables: {
      '--background': '#0D0D0D',
      '--surface': '#161616',
      '--card': '#1C1C1C',
      '--popover': '#161616',
      '--foreground': '#F0F0F0',
      '--muted': '#666666',
      '--border': '#252525',
      '--primary': '#F0F0F0',
      '--primary-hover': '#DCDCDC',
      '--accent': '#252525',
      '--color-background': '#0D0D0D',
      '--color-surface': '#161616',
      '--color-border': '#252525',
      '--color-text': '#F0F0F0',
      '--color-text-light': '#666666',
      '--color-primary-start': '#F0F0F0',
      '--color-primary-end': '#DCDCDC',
      '--color-accent-start': '#252525',
      '--color-accent-end': '#252525',
      '--color-shadow': '240, 240, 240',
    },
  },
];

export const defaultTheme = themes.find((theme) => theme.id === 'sunset') ?? themes[0];
