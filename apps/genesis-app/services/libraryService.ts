...
export const saveInfographic = async (data: InfographicData): Promise<void> => {
  try {
    ...
    if (existingIndex >= 0) {
      infographics[existingIndex] = {
        ...savedInfographic,
        savedAt: infographics[existingIndex].savedAt,
      };
    } else {
...
export const saveImage = async (
  image: Omit<SavedImage, 'id' | 'savedAt' | 'user_id'>
): Promise<void> => {
  try {
    const {
      data: { session },
...
export const getLibraryStats = async () => {
  const [infographics, images] = await Promise.all([getAllInfographics(), getAllImages()]);
