import type { BookProject, SavedBook } from '../../types';
import { supabase } from '../supabaseClient';
import { BaseApiService, type BaseEntity } from './base';
...
export class BooksApiService extends BaseApiService<BookEntity> {
  ...
  async createBook(book: SavedBook, userId: string): Promise<BookEntity> {
    ...
    const { data, error } = await supabase.from(this.table).upsert(entity).select().single();
