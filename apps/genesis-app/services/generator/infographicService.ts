...
import { MASTER_GEMINI_PROMPT, MASTER_GROK_PROMPT } from './prompts/infographicPrompts';
  ...
  main_points: [
    {
      ...
      visual_suggestion: 'Overview illustration',
      visual_metaphor: '',
      character_interaction: 'Introducing the topic',
    },
    {
      point_number: 2,
      heading: 'Key Facts',
      content: 'Important information about this topic.',
      visual_suggestion: 'Fact icons',
      visual_metaphor: '',
      character_interaction: 'Explaining facts',
    },
    {
      point_number: 3,
      ...
      visual_metaphor: '',
      character_interaction: 'Breaking it down',
    },
  ],
...
export const InfographicService = {
  ...
  generateContent: async (request: GenerationRequest): Promise<GeneratedContent> => {
    ...
    const prompt = MASTER_GROK_PROMPT.replace('{USER_TOPIC}', request.topic)
    ...
    try {
      ...
      if (
        !content.main_points ||
        !Array.isArray(content.main_points) ||
        content.main_points.length === 0
      ) {
      ...
      console.log(
        '✅ Content generated successfully with',
        content.main_points?.length || 0,
        'main points'
      );
      return content;
    ...
    try {
      ...
      const result: InfographicData = {
        ...
        content: {
          ...
          steps:
            request.type === InfographicType.PROCESS
              ? content.main_points.map((p: any) => ({
              ...
              : [],
          comparisonPoints:
            request.type === InfographicType.COMPARISON
              ? content.main_points.map((p: any) => {
              ...
              : [],
          stats:
            request.type === InfographicType.STATISTICAL
              ? content.main_points.map((p: any) => {
              ...
              : [],
          timelineEvents:
            request.type === InfographicType.TIMELINE
              ? content.main_points.map((p: any) => {
