...
class CharacterVoiceTutoringService {
  ...
  createVoiceProfile(character: Character): CharacterVoiceProfile {
    ...
    return {
      ...
      thinkingPhrases: ['Hmm, let me think...', 'Interesting question!', 'Let me explain...'],
      celebrationPhrases: ['Amazing work!', "You're a star!", 'I knew you could do it!'],
    };
  }
  ...
  startSession(character: Character, onExpressionChange?: (expression: string) => void) {
    ...
    // Greeting
    const greeting =
      character.voiceProfile?.catchphrases?.[0] ||
  ...
  private selectVoice(profile: CharacterVoiceProfile): SpeechSynthesisVoice | null {
    ...
    const genderMatch = this.voices.find(
      (v) => v.lang.startsWith('en') && v.name.includes(preferredGender)
    );
