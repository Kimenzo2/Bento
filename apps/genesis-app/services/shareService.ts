...
export const shareService = {
  ...
  async getSharedBook(shortCode: string): Promise<{
    ...
    const { data, error } = await supabase.rpc('get_book_via_share', { code_param: shortCode });
  ...
  async deleteShareLink(shortCode: string): Promise<void> {
    const { error } = await supabase.from('shared_books').delete().eq('short_code', shortCode);
