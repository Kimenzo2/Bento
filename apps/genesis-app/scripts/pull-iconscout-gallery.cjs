...
const CI = "19480773138592", CS = "HPy7qWpoCg8LQ5E9q0bsklTAK3EMYV7d";
const OUT = "C:/Users/admin/Downloads/Genesis/public/iconscout";
const CATS = [
  {name:"wand-magic",query:"magic wand",purpose:"AI generation, creative actions",lucide:"Wand2"},
  {name:"sparkle-star",query:"sparkle shine star",purpose:"Highlights, featured items",lucide:"Star"},
  {name:"book-open",query:"open book story",purpose:"Library, stories, reading",lucide:"BookOpen"},
  {name:"rocket-launch",query:"rocket launch",purpose:"Quickstart cards, achievements",lucide:"Rocket"},
  {name:"crown-premium",query:"crown premium",purpose:"Pro tier, quality badges",lucide:"Crown"},
  {name:"pen-write",query:"pen writing feather",purpose:"Authoring, editing, create",lucide:"PenTool"},
  {name:"lightning-bolt",query:"lightning bolt zap",purpose:"Quick actions, new badges",lucide:"Zap"},
  {name:"check-success",query:"checkmark circle done",purpose:"Success, completion",lucide:"Check"},
  {name:"publish-send",query:"send share upload",purpose:"Publish, share buttons",lucide:"Send"},
  {name:"library-books",query:"library shelf archive",purpose:"My Library header",lucide:"Library"},
  {name:"building-brand",query:"building office brand",purpose:"Brand Story quickstart",lucide:"Building2"},
  {name:"award-trophy",query:"award trophy medal",purpose:"Achievements, success moments",lucide:"Award"},
  {name:"art-palette",query:"paint palette art",purpose:"Art styles, Visual Studio",lucide:"Palette"},
  {name:"notification",query:"bell notification",purpose:"Notification center",lucide:"Bell"},
  {name:"story-chapter",query:"scroll manuscript book",purpose:"Story types, chapters",lucide:"FileText"},
];

function get(url){return new Promise((res,rej)=>{https.get(url,{headers:{"Client-ID":CI,"Client-Secret":CS,"User-Agent":"genesis/1"}},r=>{const c=[];r.on("data",d=>c.push(d));r.on("end",()=>res({s:r.statusCode,b:Buffer.concat(c).toString()}))}).on("error",rej)})}
...
async function main(){
  ...
  for(const cat of CATS){
    ...
    try{
      const r=await get("https://api.iconscout.com/v3/search?query="+encodeURIComponent(cat.query)+"&product_type=icon&per_page=60&formats[]=svg&sort=relevant");
      if(r.s!==200)throw new Error("HTTP "+r.s);
      const items=JSON.parse(r.b).response.items.data||[];
      console.log(items.length+" icons");total+=items.length;
      all.push({...cat,items});
    }catch(e){console.log("ERR:"+e.message);all.push({...cat,items:[]});}
    await sleep(300);
  }
  const sections=all.map(cat=>{
    const cards=cat.items.map((item,i)=>{
      const thumb=(item.urls&&(item.urls.png_256||item.urls.png_128||item.urls.png_64))||"";
      const price=item.price===0?"FREE":(item.price?"$"+item.price:"?");
      const free=item.price===0;
      return '<div class="card '+(free?"free":"paid")+'" onclick="pick(\''+item.name.replace(/'/g,"")+"','"+(item.uuid)+'\')">'+'<div class="n">'+(i+1)+"</div>"+(thumb?'<img src="'+thumb+'" loading="lazy">':"<div class='ni'>?</div>")+'<div class="nm">'+item.name.slice(0,26)+"</div>"+'<div class="pr '+(free?"f":"p")+'">'+price+"</div></div>";
    }).join("");
    return "<section><div class='ch'><b>"+cat.name+"</b> &nbsp; <code>"+cat.lucide+"</code> &nbsp; <span>"+cat.purpose+"</span> &nbsp; <em>"+cat.items.length+" icons</em></div><div class='grid'>"+cards+"</div></section>";
  }).join("\n");
