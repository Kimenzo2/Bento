...
fn draw_title_page(
    ...
    text_color(
        layer,
        &format!("Total Income:   {}", fmt_euro(overview.total_income)),
        10.0,
        MARGIN + 8.0,
        box_top + 40.0,
        font,
    ...
    text_color(
        layer,
        &format!("Total Expenses: {}", fmt_euro(overview.total_expenses)),
        10.0,
        MARGIN + 8.0,
        box_top + 27.0,
        font,
    ...
    text_color(
        layer,
        &format!("Net Savings:    {}", fmt_euro(overview.net_savings)),
        10.0,
        MARGIN + 8.0,
        box_top + 14.0,
        font,
    ...
    text_color(
        layer,
        &format!("{:.1}%", overview.savings_rate),
        20.0,
        PAGE_W / 2.0 + 20.0,
        box_top + 40.0,
        font_bold,
        if overview.savings_rate >= 20.0 {
            COLOR_GREEN
        } else {
            COLOR_AMBER
        },
    ...
    text_color(
        layer,
        &format!("{} transactions", overview.transaction_count),
        9.0,
        PAGE_W / 2.0 + 20.0,
        box_top + 10.0,
        font,
    ...
    text_color(
        layer,
        "Generated by Bento Desktop — Intelligent Budget Planner",
        7.0,
        MARGIN,
        footer_y,
        font,
...
fn draw_category_table(
    ...
    for (i, cat) in categories.iter().enumerate().take(rows) {
        ...
        let txt_color = if cat.percent_used >= 100.0 {
            COLOR_RED
        } else if cat.percent_used >= 80.0 {
            COLOR_AMBER
        } else {
            COLOR_DARK
        };
        ...
        text_color(
            ...
            text_y,
            font,
            if cat.spent > cat.monthly_budget {
                COLOR_RED
            } else {
                COLOR_DARK
            },
        ...
        text_color(
            layer,
            &fmt_int(cat.remaining),
            7.0,
            col_x[3],
            text_y,
            font,
            if cat.remaining >= 0.0 {
                COLOR_GREEN
            } else {
                COLOR_RED
            },
        ...
        let bar_color = if cat.percent_used >= 100.0 {
            COLOR_RED
        } else if cat.percent_used >= 80.0 {
            COLOR_AMBER
        } else {
    ...
    text_color(
        layer,
        "Page — Report generated by Bento Desktop",
        7.0,
        MARGIN,
        18.0,
        font,
...
fn draw_financial_health(
    ...
    text_color(
        ...
        &format!(
            "Savings Rate:    {}  —  {}",
            health.savings_rate_grade, &health.savings_rate_grade
        ),
        9.0,
        MARGIN,
        grades_y,
    ...
    text_color(
        layer,
        &format!("Budget Adherence: {}", health.budget_adherence),
        9.0,
        MARGIN,
        grades_y - 10.0,
        font,
    ...
    text_color(
        layer,
        &format!("Bill Payment:     {}", health.bill_payment_rate),
        9.0,
        MARGIN,
        grades_y - 20.0,
        font,
    ...
    text_color(
        layer,
        &format!("Debt/Income:      {}", health.debt_income_ratio),
        9.0,
        MARGIN,
        grades_y - 30.0,
        font,
...
fn draw_bills_table(
    ...
    for (i, bill) in bills.iter().enumerate().take(max_rows) {
        ...
        let status = if bill.paid_this_month {
            "Paid ✓"
        } else {
            "Pending"
        };
        let status_color = if bill.paid_this_month {
            COLOR_GREEN
        } else {
            COLOR_RED
        };
...
fn draw_ai_costs(
    ...
    if !summary.is_empty() {
        ...
        for (i, s) in summary.iter().enumerate().take(15) {
            ...
            text(
                layer,
                &format!("{}", (s.total_tokens_in + s.total_tokens_out)),
                7.0,
                col_x[2],
                row_y,
                font,
        ...
        text_color(
            layer,
            "No AI costs tracked this period.",
            10.0,
            MARGIN,
            PAGE_H - MARGIN - 60.0,
            font,
...
fn draw_forecast(
    ...
    for (i, proj) in cash_flow.iter().enumerate().take(12) {
        ...
        text_color(
            layer,
            &fmt_int(proj.projected_expenses),
            6.5,
            col_x[2],
            row_y,
            font,
        ...
        let bal_color = if proj.projected_balance >= 0.0 {
            COLOR_GREEN
        } else {
            COLOR_RED
        };
        text_color(
            layer,
            &fmt_int(proj.projected_balance),
            6.5,
            col_x[3],
            row_y,
            font,
