...
impl GenesisMcp {
    ...
    #[tool(description = "Return the static creative planning scaffold used by Genesis Desktop.")]
...
fn main() {
    let runtime = Builder::new_current_thread()
        ...
        .unwrap_or_else(|error| {
            eprintln!("genesis-mcp failed to initialize runtime: {error}");
            std::process::exit(1);
        });

    if let Err(error) = runtime.block_on(async {
        let service = GenesisMcp.serve(stdio()).await?;
        service.waiting().await?;
        Ok::<(), Box<dyn std::error::Error + Send + Sync>>(())
    }) {
        eprintln!("genesis-mcp failed: {error}");
        std::process::exit(1);
    }
}
